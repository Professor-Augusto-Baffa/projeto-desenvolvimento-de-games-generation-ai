...
func initialize_labels():
	$dayLabel.size = Vector2(game_size.x * 0.1, game_size.y * 0.1)
	$dayLabel.position = Vector2(game_size.x * 0.9, game_size.y * 0.05)
	$dayLabel.show()
	$dayLabel.text = "Day: " + str(day)
	
	$pointLabel.size = Vector2(game_size.x * 0.1, game_size.y * 0.1)
	$pointLabel.position = Vector2(game_size.x * 0.2, game_size.y * 0.05)
	$pointLabel.text = "Points: " + str(points)
	
	$timeLabel.size = Vector2(game_size.x * 0.1, game_size.y * 0.1)
	$timeLabel.position = Vector2(game_size.x * 0.8, game_size.y * 0.95)
	$timeLabel.show()
